package a.miracle.androidlib.widget.nav;
...
public class FragmentNavigator extends Navigator<FragmentNavigator.Destination> {
    // 修改源码这种方式，并不是一个特别好的解决方案。如果有其他更好的方法，方案，欢迎给我留言，讨论。
    // -----------------------------------add custom start-----------------------------------

    /**
     * 获取当前栈顶的Fragment, 虽然没怎么看懂, 但是 mContainerId 就是当前当前栈顶 Fragment Id
     * 或者可以取 mBackStack.getLast()
     * @return
     */
    ...
    public Fragment instantiateFragment(@NonNull Context context,
        ...
        if(fragment == null){
            fragment = fragmentManager.getFragmentFactory().instantiate(
                    context.getClassLoader(), className);
            Log.i(TAG, "instantiate : " + className);
        }else {
            Log.i(TAG, "findFragmentByTag : " + className);
        }
        return fragment;
    }
    ...
    public NavDestination navigate(@NonNull FragmentNavigator.Destination destination, @Nullable Bundle args,
        ...
        ft.replace(mContainerId, frag, className);
