package a.miracle.lib_widget.azlist;
...
public class AZWaveSideBarView extends View {
	...
	private void initAttribute(AttributeSet attrs, int defStyleAttr) {
		...
		mHintCircleRadius = typeArray.getDimensionPixelOffset(R.styleable.AZWaveSideBarView_hintCircleRadius,
															  (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 24,
																							  getResources().getDisplayMetrics()));
		mHintCircleColor = typeArray.getColor(R.styleable.AZWaveSideBarView_hintCircleColor, Color.parseColor("#AEDAD2"));
		mWaveColor = typeArray.getColor(R.styleable.AZWaveSideBarView_waveColor, Color.parseColor("#AEDAD2"));
	...
	private void initData() {
		mLetters = Collections.singletonList("A");
...
/*		//绘制圆角矩形
		...
		for (int index = 0; index < mLetters.size(); index++) {
			...
			if (mPreSelect == index) {
				mTextPaint.setColor(mHintCircleColor);
			}else {
