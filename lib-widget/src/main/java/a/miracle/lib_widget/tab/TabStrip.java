 ...
 */
package a.miracle.lib_widget.tab;
...
class TabStrip extends LinearLayout {
  ...
  TabStrip(Context context, AttributeSet attrs) {
    ...
    isForeground = a.getBoolean(
        R.styleable.stl_SmartTabLayout_stl_isForeground, false);
    a.recycle();
  ...
  TabLayout.TabColorizer getTabColorizer() {
    return (customTabColorizer != null) ? customTabColorizer : defaultTabColorizer;
  }

  @Override
  protected void onDraw(Canvas canvas) {
    if (!isForeground && !drawDecorationAfterTab) {
  ...
  private void drawDecoration(Canvas canvas) {
    ...
    final TabLayout.TabColorizer tabColorizer = getTabColorizer();
  ...
  private static class SimpleTabColorizer implements TabLayout.TabColorizer {
    ...
    public final int getIndicatorColor(int position) {
      if(position >= indicatorColors.length){
        return indicatorColors[indicatorColors.length - 1];
      }
